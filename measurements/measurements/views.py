...
def MeasurementCreate(request):
    if request.method == 'POST':
        ...
        if check_variable(data_json) == True and check_place(data_json) == True:
                ...
                db_measurement.variable = data_json['variable']
                db_measurement.value = data_json['value']
                db_measurement.unit = data_json['unit']
                db_measurement.place = data_json['place']
                db_measurement.save()
                return HttpResponse("successfully created measurement")
        else:
            return HttpResponse("unsuccessfully created measurement. Variable or place dont not exist")
